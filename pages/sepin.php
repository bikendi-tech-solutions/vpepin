...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Successful Exam Pin Printing</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sepins","true");
}
elseif(empty($_GET["trans_id"])){
  pagination_history_before("sepins","true");
    ...
    if(is_numeric($_GET["trans_id"])){
        $id = $_GET["trans_id"];
        pagination_history_before("sepins","true","AND id = '$id'");
    }
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("sepins","true","AND request_id = '$id'");
    }
}
...
foreach($transactions as $result){
    ...
    <td>
    <div class='input-group bg bg-danger text-white'>
...
<label class="customcheckbox mb-3">
  ...
  <select onchange="openfunction('sepins','E-Cards',false)">
